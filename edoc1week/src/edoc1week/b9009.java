...
public class b9009 {
	...
	public static void main(String[] args) {
		...
		int[] fibo = new int[45];
		//피보나치 배열 선언 생성, 1 ≤ n ≤ 1,000,000,000 이므로 45번째 피보나치 수는 1,134,903,170 이므로 배열은 45개만 생성 
		...
		for(int i=0; i<num; i++) {//num만큼(예시에서는 4) 루프 돌리기
			int t= testarr[i];//구해야되는 수 복제, 빼야 되는데 test[i]에 바로하면 값이 누적 되므로 하나 복제한다.
			
			
			for(int j=44; j<fibo.length;j--) {
				//최소 갯수의 피보나치 수들의 합으로 나타내기 때문에 큰 수부터 구해야되므로 피보나치 배열을 뒤에서부터 비교한다.
